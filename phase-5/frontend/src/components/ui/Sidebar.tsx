...
import { logout as apiLogout, getNotifications } from '@/lib/api';
...
const navItems: NavItem[] = [
  ...
  { id: 'chat', name: 'Chat', icon: <ChatIcon />, href: '/chat' },
    ...
    <>
      ...
      <div className="px-4 pt-4">
        <button
          ...
          className="relative w-full flex items-center gap-3 px-4 py-3 rounded-lg text-zinc-400 hover:bg-zinc-800 hover:text-white transition-all duration-200"
        >
          <BellIcon />
        ...
        {showNotifications && (
          <div className="mt-2 max-h-48 overflow-y-auto space-y-1 px-2">
            {notifications.length === 0 ? (
              <p className="text-xs text-zinc-500 px-2 py-1">No notifications</p>
            ) : (
              notifications.slice(0, 10).map(n => (
                <div key={n.id} className={`text-xs px-3 py-2 rounded ${n.read ? 'text-zinc-500' : 'text-zinc-300 bg-zinc-800'}`}>
                  {n.message}
                </div>
              ))
            )}
